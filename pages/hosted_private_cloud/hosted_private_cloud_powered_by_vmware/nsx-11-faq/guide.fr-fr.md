...
/// details | Quels sont les différences entre les packs d’assistance <a name="diffencepack"></a>
...
/// details | Comment puis-je protéger mes machines virtuelles directement exposées sur Internet avec une IP Publique ? <a name="public ip"></a>
...
/// details | Quelle est la date de fin de vie de NSX-v ? <a name="eofnsxv"></a>
...
/// details | Est-il possible de faire du Border Gateway Protocol (BGP) ? <a name="bgp"></a>
...
/// details | Est-ce que NSX-T est compatible BGP à travers IPSEC ? <a name="bgpoveripsec"></a>

Oui, à travers une VRF.

///

/// details | Quel est le changement concernant le système autonome BGP (AS) ?
...
/// details | Peut-on mettre un pare-feu virtuel devant le Tier-0 dans le même vSphere managé ?
...
/// details | Quelle est la différence entre une passerelle Tier-0 et une Tier-1 ? <a name="t0vst1"></a>
...
/// details | Comment puis-je ajouter une autre Tier-0 Gateway ? <a name="addt0gw"></a>
...
/// details | Le bouton "Edit" dans NSX pour une Tier-0 est désactivé, comment puis-je configurer la Gateway publique ? <a name="publicgateway"></a>
...
/// details | Puis-je configurer une Tier-0 Gateway active-active afin d’avoir une double bande passante ("garantie" 10+10=20Gb/s et "théorique" 25+25=50Gb/s) ? 
...
/// details | Est-il possible de se connecter à la Edge en SSH pour effectuer un diagnostic ou de la capture de paquets ? <a name="t0gwdoublebw"></a>
...
/// details | Quel est le nombre maximum d'interfaces (segments connectés) sur une Tier-1 Gateway ? <a name="t1interface capacity"></a>
...
/// details | Comment puis-je ajouter des IP publiques ? <a name="addpublicicip"></a>
...
/// details | Est-ce que les blocs d’adresses IP peuvent être utilisés/distribués entre deux Datacenters VMware dans un même Hosted Private Cloud ? <a name="ipblockdistribution"></a>
...
/// details | Comment sont gérées les IP lors de la migration ? <a name="managemigrationip"></a>
...
/// details | Pour la migration vDC, il faut passer le datastore en global. Un retour en arrière est-il possible sur cette configuration ? <a name="vdcmigration"></a>
...
/// details | Allez-vous fournir des formations et de la documentation pour améliorer les compétences NSX-T ? <a name="docandtrainingnsxt"></a>
...
/// details | Si une migration NSX-v a été planifiée vers une solution tierce pfSense, doit-on repartir sur une demande de création d'un nouveau Datacenter virtuel sans NSX-v ? Peut-on le faire sur le Datacenter virtuel existant ? <a name="nsxtmigrationpfsense"></a>
...
/// details | La migration des IP peut-elle se faire IP par IP ou par bloc IP ? <a name="ipmigrationperblock"></a>
...
/// details | Cette migration interrompt-elle les services portés par NSX ? Si oui, combien de temps ? <a name="ipmigrationinterruption"></a>
...
/// details | Quel sera le débit des cartes des edge-nodes, sachant que le Tier-0 (T0) sera mutualisé ? <a name="bandwidthedgenode"></a>
...
/// details | Est-ce que le vRack fonctionne avec les NSX-T ?  <a name="vrackwithnsxt"></a>
...
/// details | Le cluster de NSX Edges aura-t-il accès au vRack OVHcloud ? <a name="vrackaccess"></a>
...
/// details | Puis-je configurer la haute disponibilité sur les NSX Edges ? <a name="ha"></a>

Non, la gestion des NSX Edges est effectuée par OVHcloud en mode Actif Passif.

///

/// details | Est-il possible d'avoir plusieurs edge clusters ? <a name="multipleedgeclusters"></a>

Aujourd'hui, 1 seul cluster NSX-T Edge est déployé.

///

/// details | Actuellement, nous avons 300 edges et environ 5000 RDP simultanés. La configuration moyenne "4 vcpu / 8 Go RAM / 200 Go" va-t-elle tenir pour les flux ? <a name="averageconfiguration"></a>
...
/// details | Je viens de créer un cluster et je voudrais le configurer pour NSX en API. Comment faire ?
...
/// details | Puis-je utiliser l’API OVHcloud pour configurer et utiliser mes NSX Edges en API ? <a name="api"></a>
...
/// details | Que faire de mes options de sauvegarde Veeam et de replication Zerto ? Sont-elles toujours compatibles avec NSX ? <a name="veeamzerto"></a>

Oui, mais il faudra les reconfigurer après la migration de votre Datacenter virtuel.

///

/// details | Est-il possible de faire cohabiter NSX-v et NSX-T durant la phase de transition ? <a name="nsxtwithnsxv"></a>
...
/// details | Pouvons-nous utiliser "migration coordinator" pour la migration entre NSX-v et NSX-T ? <a name="nsxmigrationcoordinator"></a>
...
/// details | Pourquoi l'interface de gestion de NSX est-elle réalisée via un endpoint `https://nsxt.pcc-xxx-xxx-xxx-xxx.ovh.com` distinct ? <a name="nsxterraform"></a>
...
/// details | Est-il possible de faire communiquer un routeur NSX Edge entre 2 environnements Hosted Private Cloud ? <a name="nsxedge"></a>

Oui, c'est possible, via le vRack par exemple, ou via le réseau public et les VIP des Edges Cluster.

///

/// details | Réalisez-vous des backups des configs NSX-T, y compris le paramétrage manuel du client ? <a name="nsxbackupmanualconfiguration"></a>
...
/// details | Est-il possible d'avoir la livraison d'un vDC NSX-T dans un VMware vSphere managé d'année 2016, existera-t-il des problèmes lors de la livraison ? <a name="nsxtinto2016pcc"></a>

Oui, c'est possible, il n'y a pas de contraintes actuellement.

///

/// details | Des limitations supplémentaires sont-elles présentes dans un contexte SecNumCloud ? <a name="snclimitations"></a>

Il n’y a pas de limitations supplémentaires par rapport à un environnement non qualifié SecNumCloud.

///

/// details | La gestion des sauvegardes est-elle différente sur l'offre qualifiée SecNumCloud ? <a name="semnumcloudbackupoutside"></a>

Il n'existe pas de différence entre les sauvegardes dans les environnements qualifiés SecNumCloud et non SecNumCloud.

///

/// details | Pendant la phase de migration, devra-t-on payer en double notre plateforme durant un mois ? <a name="priceduringmigration"></a>
...
/// details | Pouvons nous utiliser le Load Balancer Advanced (AVI) ou le Gateway IDS/IPS ? <a name="pricealbandipsids"></a>

Non, ces fonctionnalités ne sont pas incluses.

///

/// details | Pouvons nous utiliser l'IDS/IPS distribué ? <a name="pricealbandipsids"></a>
